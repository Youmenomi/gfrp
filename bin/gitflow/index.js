...
class GitFlow extends Plugin {
  constructor({
    ...
    this.config.options = lodash.defaultsDeep({}, this.config.constructorConfig, {
      ci: isCI || undefined
  ...
  async init() {
    ...
    if (gitflow) {
      if (this.global.isCI) {
        throw new Error('failed: Git Flow workflow does not support CI mode.' + EOL + 'Alternatively, use `--no-gfrp.gitflow` to release without git-flow workflow' + ' (or save `"gfrp.gitflow": false` in the configuration).');
      }
    ...
    npm.prototype.release = async function () {
      ...
      if (this.options.tag) {
        ...
        if (npmTags) {
          ...
          Object.keys(JSON.parse((await this.exec(`npm view ${this.getName()} dist-tags -json`, {
            write: false
          })))).forEach(value => {
            choices.push({
              name: value,
      ...
      const otpCallback = this.global.isCI ? null : task => this.step({
        prompt: 'otp',
        task
      });
